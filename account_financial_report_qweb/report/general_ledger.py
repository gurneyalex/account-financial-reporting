    ...
    def _get_final_partner_sub_subquery_sum_amounts(self, only_empty_partner,
        ...
        subquery_sum_amounts = """
            ...
            SELECT
                ...
                SUM(ROUND(ROUND(COALESCE(sub.debit, 0.0), 2)) AS debit,
                SUM(ROUND(ROUND(COALESCE(sub.credit, 0.0), 2)) AS credit,
                SUM(ROUND(ROUND(COALESCE(sub.balance, 0.0), 2)) AS balance,
                MAX(ROUND(sub.currency_id) AS currency_id,
                SUM(ROUND(COALESCE(sub.balance_currency, 0.0), 2)) AS balance_currency
            FROM
            (
        """
