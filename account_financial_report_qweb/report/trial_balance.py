...
SELECT
    ...
    rpg.partner_id,
    rpg.name,
    round(rpg.initial_balance AS initial_balance, 2),
    round(rpg.initial_balance_foreign_currency, 2) AS initial_balance_foreign_currency,
    ...
    round(rpg.final_balance_foreign_currency, 2) AS final_balance_foreign_currency
FROM
...
SELECT account_group_id, code,
    ...
    sum(round(initial_balance_foreign_currency, 2)) AS initial_balance_foreign_currency,
    ...
    sum(round(final_balance_foreign_currency, 2)) AS final_balance_foreign_currency
FROM   cte
...
(SELECT
    ...
    sum(round(coalesce(ra.debit, 0),2)) - sum(round(coalesce(ra.credit, 0), 2)) as period_balance,
